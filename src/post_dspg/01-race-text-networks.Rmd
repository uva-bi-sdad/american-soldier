...
edgelist_simplified <- comb_edgelist_wn %>% select(source, target) %>% rename(Source = source, Target = target)
edgelist_attributes <- comb_edgelist_wn %>% select(weight, group) %>% rename(Weight = weight, Group = group)
gexf_file <- write.gexf(nodes=combined_nodelist, edges=edgelist_simplified, edgesAtt=edgelist_attributes, 
                        output = "~/git/american-soldier/data/post_dspg/network.gexf")
